...
class RoPE(nn.Module):
    def __init__(self, d_model=d_model, max_len=max_len):
        ...
        self.register_buffer('sin', torch.sin(sinusoid_inp).unsqueeze(0))
        self.register_buffer('cos', torch.cos(sinusoid_inp).unsqueeze(0))
...
class Bert_like(nn.Module):
    def __init__(self, d_model=d_model, nhead=nhead, num_encoder_layers=num_encoder_layers, 
        ...
        self.classifier = nn.Linear(d_model, 37)
...
def vector_to_label(vector):
    onehot_part = vector[:34]
    tile_index = torch.argmax(onehot_part).item()
